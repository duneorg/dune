...
MDX content can use custom JSX components that aren't standard HTML elements. These components must be registered before the engine starts using the programmatic API:
...
import { Alert } from "./themes/my-theme/components/Alert.tsx";
import { Chart } from "./themes/my-theme/components/Chart.tsx";

// 1. Create a registry and register components
const registry = createMdxComponentRegistry({
  Alert,
  Chart,
});

// 2. Build a format registry with the configured MDX handler
const formats = new FormatRegistry();
formats.register(new MdxHandler({ components: registry }));

// 3. Pass the format registry to createDuneEngine
const engine = await createDuneEngine({ storage, config, formats });
```
