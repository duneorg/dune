...
**There is no `providers.localUsers` config, at any userStore tier** — self-service signup is still only OAuth or magic link. But an admin *can* now pre-provision an account ahead of the person's first login: `dune users:create <email> [--role x[,y]] [--name "..."]` creates a `User` record directly (`provider: "invited"`). The invited person's first login via magic link matches it by email and just works; a first login via OAuth 409s instead (no linked provider yet on a record nobody's ever actually authenticated as) — see "Linking an additional provider" below for the fix.
...
GET  /auth/{provider}/link         → begin OAuth to LINK this provider to the current session's account (401 if not logged in)
GET  /auth/{provider}/callback     → complete OAuth — either an ordinary login/signup, or completing a /link flow above
POST /auth/{provider}/unlink       → remove a previously-linked (non-primary) provider (401 if not logged in)
...
### Linking an additional provider to an existing account

Any logged-in user can connect another OAuth provider to their account — not just for `users:create`-provisioned accounts, any account (e.g. one that originally signed up via magic link and now also wants GitHub login):

```html
<a href="/auth/github/link">Connect GitHub</a>
```

`getByProvider()` checks both the account's original `provider`/`providerId` (its signup method — never removable through this flow) and everything in `linkedProviders` (added via `/link`, removable via `POST /auth/{provider}/unlink`). `GET /auth/me` returns linked provider *names* (not IDs) as `linkedProviders: string[]`.

**If the provider identity you're linking already belongs to a *different* account**, `/callback` doesn't error — it logs you into that account instead (redirects to `/?dune_link=linked_elsewhere`), since completing that provider's OAuth consent screen is real proof you control it. Check for `dune_link` in the URL (`linked` | `already_linked` | `linked_elsewhere`) to show your own message; there's no built-in UI for this.

**Unlinking** is blocked when it would remove your only *additional* identity while magic link is disabled site-wide (your original signup provider is never at risk either way — it's not removable through this route at all).
...
**No passwords, still no `providers.localUsers`.** Password auth is out of scope for public-site accounts. Self-service signup is still only OAuth or magic link — but `dune users:create` (CLI) *can* pre-provision an account ahead of someone's first login; see "With persistent user records + magic link" above.

**Account linking only works for `userStore: local`/`db`, not `userStore: session`.** `GET /auth/{provider}/link` 400s under `userStore: session` — there's no persistent record to attach a linked identity to (identity there is synthesized fresh from OAuth claims every login).
