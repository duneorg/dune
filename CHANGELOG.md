...
- **External JWTs without an `exp` claim never expired.** `verifyExternalJwt()`
  only enforced expiry when `exp` was present; a token minted without it (or a
  leaked one) stayed valid forever. `exp` is now required and must be numeric.
- **Magic link tokens were multi-use for their full 15-minute lifetime unless
  the caller supplied a `MagicTokenStore`** — only the built-in auth routes
  wired one in, so direct library callers got replayable login links.
  `verifyMagicToken()` now defaults to a shared in-memory store, so tokens are
  single-use even without one (multi-process deployments should still pass an
  explicit KV/Redis-backed store).
- **Uploads were accepted on extension alone.** A payload renamed to `.gif` or
  `.png` passed the allowlist and could be stored and later replayed with a
  trusted image content-type. Upload handling now verifies the file's leading
  bytes against its server-derived MIME type (JPEG, PNG, GIF, WEBP, AVIF, PDF,
  and ZIP-based Office formats) and rejects mismatches.
- **The SSRF guard didn't cover several special-use IPv4/IPv6 ranges** — IETF
  protocol assignments, the TEST-NET/benchmarking/documentation ranges, reserved
  class E, the NAT64 well-known prefix, the discard-only range, and
  IPv4-compatible/embedded literals are now blocked alongside the
  previously-covered private/loopback/link-local ranges.
- **The session-userStore-plus-payments startup warning never fired.** It
  checked a `payments.enabled` field that doesn't exist on the config type
  (payments are activated by a configured `secret_key` under `site.payments`);
  the check now looks at the right field.
- **Stripe webhook parsing silently dropped body-read and JSON-parse failures**,
  and a failed `authz.addMember()` role grant was only logged via
  `console.warn`. All three now go through the structured logger, so a dropped
  `checkout.session.completed` event is actually visible instead of a paying
  customer silently never getting their role.
- **Freshly scaffolded sites pinned an outdated `@dune/core` range and were
  missing the `@std/encoding/hex` import-map entry** other core modules already
  depend on directly. New sites now scaffold with a pin matching the core
  version they were generated by and a complete import map. Existing sites are
  unaffected — `dune upgrade` still rewrites the pin the same way it always has.
- **`dune new` gave no indication why `deno task dev` failed** when the new
  site was scaffolded inside a directory covered by an enclosing Deno
  workspace that didn't list it as a member — just a bare `Config file must
  be a member of the workspace` error from Deno itself. `dune new` now
  detects that situation and prints the exact `workspace` array entry to add
  and where. (duneorg/dune#6)
- **The starter theme shipped no error template**, so every 404/500 on a fresh
  scaffold fell back to bare markup instead of a branded page. **The scaffolded
  blog page also declared a collection query the default template never
  rendered**, so a fresh site's blog listing was empty regardless of how many
  posts existed. `dune new` now scaffolds `templates/error.tsx` and
  `templates/blog.tsx` and points the blog page at the latter.

### Added

- **`ownershipError(user, ownerId)`** (`@dune/core/auth/api-guard`) —
  centralizes the ownership check for hand-written `mode: "owner"` CRUD routes
  (401 unauthenticated, 403 on mismatch or a malformed owner id), so it doesn't
  have to be reimplemented per handler. Generated CRUD routes already scope
  correctly on their own; this is for routes written by hand.

### Changed

- **Plugin and theme `jsr:`/`npm:` specifiers no longer require an exact version
  pin — a `^`/`~` range is accepted too.** The exact-pin-only rule
  (`assertPinnedPluginSpecifier()`, `assertPinnedThemeSpecifier()`) shipped to
  close a real gap — no version check at all — but the "exact only" shape turned
  out to structurally conflict with `minimumDependencyAge`: an exact pin names
  one version, so if that version is younger than the configured age it fails
  outright, with no older-but-still-matching version to fall back to the way a
  range allows. A site that wanted both protections (pin discipline and an age
  gate) had no way to satisfy both. `--lock --frozen` (dune's default) already
  freezes whatever version a range resolves to on first sync and refuses to
  silently re-resolve later, so drift is caught by the lockfile regardless of
  whether the declared specifier is exact or a range — the same reasoning
  already applied to `@dune/plugin-admin`'s own bootstrap import (see the
  supply-chain backlog). Requiring an exact pin on top of that bought marginal
  defense-in-depth at the cost of composability; the check is now: name _a_
  version, exact or ranged, and let the lockfile do the anti-drift work.
