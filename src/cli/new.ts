...
const MCP_JSON = `{
  "mcpServers": {
    "dune": {
      "command": "deno",
      "args": ${JSON.stringify(ENTRYPOINT_MCP_ARGS)},
      "cwd": "."
    }
  }
}
...
const DENO_JSON = `{
  "imports": {
    ...
    "@dune/core": "jsr:@dune/core@^0.33",
${Object.entries(DUNE_CORE_RUNTIME_IMPORTS).map(([k, v]) => `    "${k}": "${v}"`).join(",\n")}
  },
  "tasks": {
${Object.entries(ENTRYPOINT_TASKS).map(([k, v]) => `    "${k}": "${v}"`).join(",\n")}
  },
  "compilerOptions": {
    "jsx": "react-jsx",
...
export async function newCommand(dir: string, options: { headless?: boolean } = {}) {
  ...
  await Deno.writeTextFile(`${dir}/main.ts`, MAIN_TS_TEMPLATE);
